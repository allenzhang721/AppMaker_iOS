...
#define kURBAlertBackgroundRadius 10.0      //10
#define kURBAlertFrameInset 7.0             //7
#define kURBAlertPadding 8.0                //8
#define kURBAlertButtonPadding 9.0          //9
#define kURBAlertButtonHeight 43.0          //44
#define kURBAlertButtonOffset 7             //7
#define kURBAlertTextFieldHeight 29.0       //29
...
//			self.cancelButtonBackgroundColor = [UIColor blueColor];
//		}
//		
//		[self.buttons enumerateObjectsUsingBlock:^(UIButton *button, NSUInteger idx, BOOL *stop) {
...
//		_cancelButtonBackgroundColor = [UIColor blueColor];
//		
//		if (self.cancelButton) {
...
- (NSInteger)addButtonWithTitle:(NSString *)title {	
	...
	UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, kURBAlertButtonHeight)];
...
- (NSInteger)addButtonWithTitle:(NSString *)title sideLeft:(BOOL)left {
	// convert button over to internal button
	UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 127, kURBAlertButtonHeight)];
    if (left == YES)
    {
        //    [button setBackgroundImage: [UIImage imageNamed:@"ipad_btn.png"] forState:UIControlStateNormal];
        [button setBackgroundImage:[UIImage imageNamed:@"ipad_btn_down_left.png"] forState:UIControlStateHighlighted];
    }
    else
    {
        //    [button setBackgroundImage: [UIImage imageNamed:@"ipad_btn.png"] forState:UIControlStateNormal];
        [button setBackgroundImage:[UIImage imageNamed:@"ipad_btn_down_right.png"] forState:UIControlStateHighlighted];
    }
...
- (void)build {
    ...
    if (!self.backgroundView) {
		...
		_backgroundView.autoresizingMask = UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleLeftMargin;
...
- (void)layoutComponents {
    self.bounds =CGRectMake(self.bounds.origin.x, self.bounds.origin.y, self.backgroundImage.size.width, self.backgroundImage.size.height);
	...
	layout.titleRect = CGRectMake(0, minY - 18, layoutWidth, titleHeight);
...
//	self.messageLabel.text = self.message;
    ...
    //	layout.textFieldsRect = CGRectMake(13, 80, layoutWidth - kURBAlertPadding * 4, textFieldsHeight);
    layout.textFieldsRect = CGRectMake(13, 55, layoutWidth - kURBAlertPadding * 4, textFieldsHeight);
    ...
    CGRect backgroudFrame = CGRectMake(10, 0, self.backgroundImage.size.width, self.backgroundImage.size.height);
    self.backgroundView.frame = backgroudFrame;
    ...
    layout.buttonRect = CGRectMake(CGRectGetMinX(layoutFrame) - layoutFrameInset, CGRectGetMinY(layout.buttonRegionRect) + buttonRegionPadding, layoutWidth, buttonsHeight);
	// adjust layout frame
	...
	if (fieldCount > 0) {
		for (int i = 0; i < fieldCount; i++) {
			...
			CGRect frame = CGRectIntegral(CGRectMake(CGRectGetMinX(layout.textFieldsRect),
                                                     yOffset ,
                                                     CGRectGetWidth(layout.textFieldsRect),
                                                     kURBAlertTextFieldHeight));
			
			UITextField *field = (UITextField *)[self.textFields objectAtIndex:i];
			field.frame = frame;
			field.autoresizingMask = UIViewAutoresizingFlexibleWidth;
//            NSString *input = [[NSUserDefaults standardUserDefaults] valueForKey:@"userinput"];
//            field.text = input;
	...
	if (count > 0) {
		...
		for (int i = 0; i < count; i++) {
			CGFloat xOffset = CGRectGetMinX(layout.buttonRect) + (kURBAlertButtonPadding + buttonWidth + 5) * (CGFloat)i;
			CGRect frame = CGRectIntegral(CGRectMake(xOffset + 10,
                                                     CGRectGetMinY(layout.buttonRect),
                                                     buttonWidth,
                                                     CGRectGetHeight(layout.buttonRect)));
			
			UIButton *button = (UIButton *)[self.buttons objectAtIndex:i];
			button.frame = CGRectMake(frame.origin.x + 6.2, frame.origin.y - 4, 128, 43);
...
@implementation URBAlertWindowOverlay       //背景阴影
