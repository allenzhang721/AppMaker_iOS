...
{
    TBXMLElement *ele = [EMTBXML childElementNamed:@"BOOKID" parentElement:element];
    self.tempid           = [EMTBXML textForElement:ele];
    ele                   = [EMTBXML childElementNamed:@"DOWNLOADURL" parentElement:element];
    self.downloadurl      = [EMTBXML textForElement:ele];
    ele                   = [EMTBXML childElementNamed:@"COVERURL" parentElement:element];
    self.coverurl      = [EMTBXML textForElement:ele];
    
    if ([EMTBXML childElementNamed:@"BOOKNAMEURL" parentElement:element])
    {
        ele                   = [EMTBXML childElementNamed:@"BOOKNAMEURL" parentElement:element]; //Mr.chen, 2.25,
        self.bookNameUrl      = [EMTBXML textForElement:ele];
    }
    else
    {
        self.bookNameUrl = @"";
    }
    ele                   = [EMTBXML childElementNamed:@"ISDOWNLOADED" parentElement:element];
    self.isDownloaded     = [[EMTBXML textForElement:ele] boolValue];
    ele                   = [EMTBXML childElementNamed:@"ISUNZIPED" parentElement:element];
    self.isUnziped        = [[EMTBXML textForElement:ele] boolValue];
    ele                   = [EMTBXML childElementNamed:@"ISFAIL" parentElement:element];
    self.isFail           = [[EMTBXML textForElement:ele] boolValue];
    ele                   = [EMTBXML childElementNamed:@"ISCOVERDOWNLOADED" parentElement:element];
    self.isCoverDownloaded   = [[EMTBXML textForElement:ele] boolValue];
    
    if ([EMTBXML childElementNamed:@"ISBOOKNAMEDOWNLOADED" parentElement:element]) {
        
        ele                   = [EMTBXML childElementNamed:@"ISBOOKNAMEDOWNLOADED" parentElement:element];    //Mr.chen, 2.25,
        self.isBookNameDownloaded   = [[EMTBXML textForElement:ele] boolValue];
    }
    else
    {
...
//            self.fileDownloader.indicatorLabel = self.cell.prgogreeLabel;
    ...
    if ([HLFileUtility checkFileAtPaht:[self getCoverPath]] == YES)
    {
        [self onCoverdownloadsuccess:[self getCoverPath]];
    }
    
    if ([HLFileUtility checkFileAtPaht:[self getBookNamePath]] == YES)        //Mr.chen, 2.25,
    {
    ...
    if (self.cell != nil)
    {
        if (cell.entity == self)
        {
            ...
            if (self.cell.nameLabel.text == nil || [self.cell.nameLabel.text isEqualToString:@""]) {
                ...
                self.cell.nameLabel.text = @"appMaker";
            }
        }
    }
    [self downloadCover];
...
-(void) remove
{
    ...
    [HLFileUtility delFileAtPath:[self getFilePath:self.tempid]];
}
