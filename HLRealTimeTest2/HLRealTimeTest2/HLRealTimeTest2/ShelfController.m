...
- (id)init
{
    ...
    if (self)
    {
        ...
        //删除按钮点击提示
        {
            ...
            [self.alertView setBackgroundImage:[UIImage imageNamed:@"deleteinfo.png"]];
            ...
            [self.alertView addButtonWithTitle:@"删除" sideLeft:NO];
        ...
        //Info按钮点击提示,aboutView
        {
            ...
            {
                [self.infoAlertView setBackgroundImage:[UIImage imageNamed:@"iphone_about"]];
            }
            else
            {
                [self.infoAlertView setBackgroundImage:[UIImage imageNamed:@"about.png"]];
            }
...
//        [[[App instance] getAppMaker] openBookWithRootViewController:nil bookDirectoryPath:bookpath theDelegate:nil hiddenBackIcon:NO hiddenShareIcon:NO];
    ...
    {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"注意" message:@"书籍内容出错！" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alertView show];
...
- (void)GMGridView:(GMGridView *)gridView didTapOnItemAtIndex:(NSInteger)position;
{
    ...
    {
        if ([entity.tempid isEqualToString:@"appMaker"] == YES)
        {
...
{
    ShelfBookEntity *entity = [self.shelfEntity.books objectAtIndex:index];
    if ([entity.tempid isEqualToString:@"appMaker"])
    {
        return NO;
    }
    return YES;
