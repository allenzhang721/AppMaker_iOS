...
- (void)bts_startWiggling
{
    ...
    [rotationAnimation setDuration:0.05];
    ...
    [translationXAnimation setDuration:0.05];

    [translationXAnimation setAutoreverses:YES];
    [translationXAnimation setFromValue:[NSNumber numberWithFloat:[self bounds].origin.x + 4.0]];
    [translationXAnimation setToValue:[NSNumber numberWithFloat:[self bounds].origin.x - 4.0]];
    ...
    [self performSelector:@selector(bts_stopWiggling) withObject:nil afterDelay:.4];
}
