...
-(void) load
{
    ...
    if (data != nil)
    {
        ...
        if (root)
        {
            TBXMLElement *bks = [EMTBXML childElementNamed:@"BOOKS" parentElement:root];
            TBXMLElement *book = [EMTBXML childElementNamed:@"BOOK" parentElement:bks];
            while(book != nil)
            {
                ShelfBookEntity *entity = [[[ShelfBookEntity alloc] init] autorelease];
                [entity decode:book];
                [self.books addObject:entity];
                book =  [EMTBXML nextSiblingNamed:@"BOOK" searchFromElement:book];
            }
        }
        [tbxml release];
    }
