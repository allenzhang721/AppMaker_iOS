...
@interface HLZipArchive (Private)
...
//	NSDictionary* attr = [[NSFileManager defaultManager] fileAttributesAtPath:file traverseLink:YES];
    NSDictionary* attr = [[NSFileManager defaultManager] attributesOfItemAtPath:file error:nil];        //colin modify
...
//            NSLog(@"%@", [NSString stringWithFormat:@"%d entries in the zip file",globalInfo.number_entry] );       //colin modify
		}
	}
...
//		NSString * strPath = [NSString  stringWithCString:filename];
        NSString * strPath = [NSString  stringWithUTF8String:filename];     //colin use stringWithUTF8String instead of stringWithCString
