...
@interface HLFlipBaseController ()        //陈星宇
...
@implementation HLFlipBaseController
...
-(BOOL) gotoPageWithPageID:(NSString *)pageid animate:(Boolean)animate
{
    ...
    if(index!= -1)
    {
        pageEntity = [HLPageDecoder decode:[self.sectionPages objectAtIndex:index] path:self.rootPath];
    }
    else
    {
        ...
        {
            HLPageEntity *currentPage = [HLPageDecoder decode:self.currentPageid path:self.rootPath] ;
            ...
            {
                pageEntity =  [HLPageDecoder decode:pageid path:self.rootPath];
                ...
                if (pageEntity != nil)
                {
                    if (self.bookEntity.isVerHorMode && pageEntity.isVerticalPageType != [self isDeviceVertical])
                    {
                        ...
                        {
                            pageEntity = [HLPageDecoder decode:pageEntity.linkPageID path:self.rootPath];
                        }
                    }
                }
            }
...
- (void) confireGotoPageWithPageID:(NSString *)pageid animate:(Boolean) animate{
    ...
    else
    {
        ...
        if ([currentPage.linkPageID isEqualToString:pageid] == NO)
        {
            HLPageEntity *pe =  [HLPageDecoder decode:pageid path:self.rootPath];
...
{
    HLPageViewController *pageViewController = self.behaviorController.pageController.pageViewController;
...
-(void) stopBackgroundMusic
{
    if(self.backgroundMusic != nil)
    {
        ...
        if ([self.bookViewController.bookController.controlPanelViewController isKindOfClass:[HLBasicControlPanelViewController class]])
        {
            ((HLBasicControlPanelViewController *)self.bookViewController.bookController.controlPanelViewController).btnBgMusic.selected = YES;
        }
    }
}
...
-(void) playBackgroundMusic
{
    if(self.backgroundMusic != nil)
    {
        ...
        if ([self.bookViewController.bookController.controlPanelViewController isKindOfClass:[HLBasicControlPanelViewController class]])
        {
            ((HLBasicControlPanelViewController *)self.bookViewController.bookController.controlPanelViewController).btnBgMusic.selected = NO;
        }
    }
}
