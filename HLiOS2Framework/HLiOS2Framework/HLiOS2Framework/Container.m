...
-(void) setSpotInContainer
{
    ...
    for (int i = 0; i < [self.entity.behaviors count]; i++)
    {
        ...
        {
            Container *ref = [self.pageController getContainerByID:behavior.behaviorValue];//热区
            if (ref != nil)
            {
...
-(BOOL) checkEnterSpot:(BehaviorEntity *)behavior
{
    CGRect refrect       = self.component.uicomponent.frame;
    //    [self.inSpotContainers removeAllObjects];
    Container *ref = [self.pageController getContainerByID:behavior.behaviorValue];//热点区域
...
-(BOOL) checkOutSpot:(BehaviorEntity *)behavior
{
    CGRect refrect       = self.component.uicomponent.frame;
...
//    NSLog(@"OntouchBegin = %@",NSStringFromCGPoint(self.comStartPoint));
    if(self.entity.beLinkageArray.count !=0)
    {
        ...
        {
            LinkageEntity *linkEntity = [self.entity.beLinkageArray objectAtIndex:i];
            NSString * linkID = linkEntity.linkID;
            Container *linkContainer =  [self.pageController getContainerByID:linkID];
