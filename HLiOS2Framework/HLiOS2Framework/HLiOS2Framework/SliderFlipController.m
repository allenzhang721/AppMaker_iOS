...
@interface SliderFlipController ()
...
@implementation SliderFlipController
...
- (id)init
{
    ...
    if (self)
    {
        ...
        {
            self.page1Controller = [[SliderPageController alloc] init];                     //10.30
            [page1Controller release];
        }
        if (self.page2Controller == nil)
        {
            self.page2Controller = [[SliderPageController alloc] init];
            [page2Controller release];
        }
        if (self.page3Controller == nil)
        {
            self.page3Controller = [[SliderPageController alloc] init];
        ...
        sliderContainerController = [[SliderContainerController alloc] init];
        canScroll = YES;
        ...
        changeRightCover = NO;
        endDecelerate = YES;
        self.coverController = [[SliderCoverController alloc] init];
...
-(void) onLaunched
{
    ...
    NSString *pageId = ((SliderPageController *)self.currentPageController).currentPageEntity.entityid;
    [[NSNotificationCenter defaultCenter] postNotificationName:KNOTIFICATION_PAGECHANGE object:pageId];
    
    //      >>>>> 1.2
    [self.coverController initEntity:((SliderPageController *)self.currentPageController).currentPageEntity];
    self.currentPageController.coverPageController = ((SliderCoverController *)self.coverController).currentPageController;
    //      <<<<<
    [self beginView];
}
...
{
    //      >>>>> 1.3
    NSString *curBecoverID = ((SliderCoverController *)self.coverController).currentPageController.currentCoverPageid;
    ...
    switch (direction)
    {
        case CoverDirectionRight:
        {
            if (isGotoPage == NO) //手动翻页
            {
                ...
                {
                    pageEntity = [PageDecoder decode:[self.sectionPages objectAtIndex:(self.currentPageIndex + 1)] path:self.rootPath];
                }
            }
            else //跳转翻页
            {
                if (self.currentPageIndex <= self.sectionPages.count - 1)
                {
                    pageEntity = [PageDecoder decode:[self.sectionPages objectAtIndex:(self.currentPageIndex)] path:self.rootPath];
                }
            }
        }
            break;
...
- (void)scrollViewWillBeginDecelerating:(UIScrollView *)scrollView
{
    ...
    //    >>>>>  1.3
    [((SliderCoverController *)self.coverController) scrollViewWillBeginDecelerating:self.scrollView];
    //    <<<<<
}
...
- (void)confireGotoPageWithPageID:(NSString *)pageid animate:(Boolean) animate
{
    ...
    if ([self.currentPageController gotoPageWithPageID:pageid animate:animate] == NO)   //横向切换页
    {
        ...
        SliderPageController *mp = [self middlePage];
        SliderPageController *lp = [self lastPage];
        self.isBusy = YES;
            ...
            if (index > self.currentPageController.pageIndex)                                   //需要加载右页，
            {
                ...
                if (self.currentPageController == fp) //当前页为第一页
                {
                    ...
                    if (index < pageCount || pageCount == 1) //有后页
                    {
                        ...
                        [(SliderCoverController *)self.coverController scrollViewBeginGoToPage:self.scrollView];
...
//                                NSLog(@"6666666");
        ...
        {
//            NSLog(@"else");
            PageEntity *pe = [self searchSubPageInSection:pageid];
            if (pe != nil) // 跳子页
            {
...
//    [self.coverController clean];
    ...
    if (self.currentPageController == mp)   //跳转后是中间页
    {
        ...
        {
            PageEntity *pageEntity = [PageDecoder decode:[self.sectionPages objectAtIndex:(self.currentPageController.pageIndex - 1)] path:self.rootPath];
            [fp loadPage:pageEntity];
...
-(SliderPageController*) firstPage   //scrollView最前页
{
...
-(SliderPageController*) middlePage  //scrollView内中间页
{
    float p1dx = self.page1Controller.view.frame.origin.x;
    float p2dx = self.page2Controller.view.frame.origin.x;
...
-(SliderPageController *) lastPage  //scrollView内最后页
{
    float p1dx = self.page1Controller.view.frame.origin.x;
    float p2dx = self.page2Controller.view.frame.origin.x;
...
-(PageEntity *) searchSubPageInSection:(NSString *) pageid   //查找子页
{
    for (int i = 0; i < [self.sectionPages count]; i++)
    {
        PageEntity *pe = [PageDecoder decode:[self.sectionPages objectAtIndex:i] path:self.rootPath];
...
//no
-(SliderPageController*) searchCurrentSliderPage   //找到当前屏幕内的ViewController
{
...
-(void) arrangePage  //重新排列
{
    ...
    if (self.currentPageController == [self firstPage])
    {
        if (self.currentPageIndex > 0 ) //交换位置
        {
            ...
            PageEntity *pageEntity = [PageDecoder decode:[self.sectionPages objectAtIndex:self.currentPageIndex - 1] path:self.rootPath];
            [lp loadPage:pageEntity];
...
-(void) loadPages
{
    if (self.currentPageIndex == 0)
    {
        ...
        {
            PageEntity *pageEnity2 = [PageDecoder decode:[self.sectionPages objectAtIndex:(self.currentPageIndex+1)] path:self.rootPath] ;
...
-(void) changeToOrientation:(UIInterfaceOrientation)interfaceOrientation
{
    ...
    if(self.currentPageController.currentPageEntity.linkPageID != nil && (![self.currentPageController.currentPageEntity.linkPageID isKindOfClass:[NSNull class]] && ![self.currentPageController.currentPageEntity.linkPageID isEqualToString:@""]))
    {
        if (self.currentPageController.currentPageEntity.isVerticalPageType != isVertical)
        {
            ...
            SliderPageController *mp = [self middlePage];
            SliderPageController *lp = [self lastPage];
            PageEntity *entity  = [PageDecoder decode:self.currentPageController.currentPageEntity.linkPageID path:self.rootPath];
