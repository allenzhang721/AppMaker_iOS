...
@interface SliderPageController ()
...
@implementation SliderPageController
...
//            self.pageScrollView = [[[UIScrollView alloc] init] autorelease];
        ...
        sliderContainerController = [[SliderPageContainerController alloc] init];
    ...
    switch (direction)
    {
        case relativeDirectionDown:
        {
            if (isGotoPage == NO) //手动翻页
            {
                ...
                {
                    pageEntity = [PageDecoder decode:[self.currentPageEntity.navPages objectAtIndex:(self.subPageIndex + 1)] path:self.rootPath];
                }
            }
            else //跳转翻页
            {
...
//    [self goToPageEnableAction];
    ...
    else
    {
        ...
        [self.currentPageController loadEntity:self.currentPageController.currentPageEntity];   //必要步骤2/3
...
-(Boolean) gotoPageWithPageID:(NSString *)pageid animate:(Boolean)animate;
{
    ...
    PageController *fp = [self firstPage];
    PageController *mp = [self middlePage];
    PageController *lp = [self lastPage];
    ...
    if(index != self.subPageIndex)
    {
        ...
        if (index > self.subPageIndex)
        {
            if (self.currentPageController == fp)
            {
                ...
                if (index < pageCout)
                {
                    gotoPageEntity = [PageDecoder decode:[self.currentPageEntity.navPages objectAtIndex:index] path:self.rootPath];
                    [mp clean];
                    ...
                    [(SliderCoverPageController *)self.coverPageController scrollViewBeginGoToPage:self.pageScrollView];
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    ...
    if (self.currentPageController == mp)
    {
        ...
        {
            PageEntity *pageEntity = [PageDecoder decode:[self.currentPageEntity.navPages objectAtIndex:(self.subPageIndex - 1)] path:self.rootPath];
            [fp clean];
...
-(void) refreshContent
{
    ...
    {
        PageEntity *pageEntity = [PageDecoder decode:[self.currentPageEntity.navPages objectAtIndex:0] path:self.rootPath];
