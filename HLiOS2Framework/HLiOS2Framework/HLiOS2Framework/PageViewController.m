...
//    NSLog(@"%@",@"LoadPageEntity");
    ...
    if (entity.entityid) {
        ...
        NSArray *contentOffsetArray = [[EMDefaultRecord shareRecord] objectForKey:entity.entityid];
...
- (void)viewDidLoad
{
    ...
    if (isClear == YES)
    {
        self.curScrollView = [[ClearScrollView alloc] init];        //1.2
    }
    else
    {
...
{
    for (Container *container in self.pageController.objects)
    {
    ...
    [[EMDefaultRecord shareRecord] setObject:@[@(contentOffset.x), @(contentOffset.y)] ForKey:pageID];
...
- (void)timerUpdate
{
    if (curContainerArr.count > 0 && [UIAccelerometer sharedAccelerometer].delegate)
    {
        ...
        {
            UIView *moveImg = ((Container *)[curContainerArr objectAtIndex:i]).component.uicomponent;
            if (moveImg)
            {
...
-(void)accelerometer:(UIAccelerometer *)accelerometer didAccelerate:(UIAcceleration *)acceleration
{
    ...
    if (!isInit)
    {
        ...
        for (ContainerEntity *containerEntity in self.pageController.currentPageEntity.containers)
        {
