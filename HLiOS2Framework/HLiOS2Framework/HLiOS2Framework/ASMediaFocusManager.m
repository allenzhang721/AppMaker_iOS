...
//    return decompressedImage;
    ...
    int width  = 0;
    int height = 0;
    frame = [self.delegate mediaFocusManager:self finalFrameforView:self.mediaView];
    
    CGImageRef imageRef = [image CGImage];
 
    height = frame.size.height;
    width  = image.size.width * frame.size.height/image.size.height;
    if (width > frame.size.width)
    {
        width = frame.size.width;
        height = image.size.height*frame.size.width / image.size.width;
    }
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
...
- (ASMediaFocusController *)focusViewControllerForView:(UIView *)mediaView
{
    ...
    viewController.contentView.frame = CGRectMake(0, 0, 1024, 768);
...
//    return;
    ...
    float width  = focusViewController.mainImageView.image.size.width * frame.size.height / focusViewController.mainImageView.image.size.height;
    if (width > frame.size.width)
    {
        width = frame.size.width;
        height = focusViewController.mainImageView.image.size.height * frame.size.width / focusViewController.mainImageView.image.size.width;
    }
    frame = CGRectMake((1024 - width) / 2, (768 - height) / 2, width, height);
...
//                         frame = [self.delegate mediaFocusManager:self finalFrameforView:mediaView];
                         ...
                         scalFrame = (self.elasticAnimation?[self rectInsetsForRect:frame ratio:-kAnimateElasticSizeRatio]:frame);
                         // Trick to kep the right animation on the image frame.
                     ...
                     completion:^(BOOL finished) {
                         if(self.elasticAnimation)
                         {
                             [UIView animateWithDuration:self.animationDuration*kAnimateElasticDurationRatio
                                              animations:^{
                                                  imageView.frame = frame;//focusViewController.contentView.bounds;
                                              }
                              completion:^(BOOL finished) {
...
{
    [self.focusViewController.view removeFromSuperview];
    [self.focusViewController release];
    [super dealloc];
}
