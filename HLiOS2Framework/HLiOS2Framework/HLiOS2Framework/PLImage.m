...
{
	return [[[PLImage alloc] initWithCGImage:image] autorelease];   //陈星宇，10.28，potential leak
}

+ (id)imageWithSize:(CGSize)size
{
	return [[[PLImage alloc] initWithSize:size] autorelease];   //陈星宇，10.28，potential leak
}

+ (id)imageWithDimensions:(int)width :(int)height
{
	return [[[PLImage alloc] initWithDimensions:width :height] autorelease];   //陈星宇，10.28，potential leak
}

+ (id)imageWithPath:(NSString *)path
{
	return [[[PLImage alloc] initWithPath:path] autorelease];   //陈星宇，10.28，potential leak
}

+ (id)imageWithSizeZero
{
	return [[[PLImage alloc] init] autorelease];   //陈星宇，10.28，potential leak
}
