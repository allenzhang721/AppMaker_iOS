...
@implementation HLASIInputStream

+ (void)initialize
{
	if (self == [HLASIInputStream class]) {
		readLock = [[NSLock alloc] init];
	}
}

+ (id)inputStreamWithFileAtPath:(NSString *)path request:(HLASIHTTPRequest *)theRequest
{
	HLASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(HLASIHTTPRequest *)theRequest
{
	HLASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	...
	if ([HLASIHTTPRequest isBandwidthThrottled]) {
		toRead = [HLASIHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	if (rv > 0)
		[HLASIHTTPRequest incrementBandwidthUsedInLastSecond:rv];
	return rv;
}
