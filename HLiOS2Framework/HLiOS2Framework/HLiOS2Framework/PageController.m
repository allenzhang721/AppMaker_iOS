...
-(void) setup:(CGRect) rect WithClear:(BOOL)clear
{
    if (pageViewController == nil)
    {
        pageViewController = [[PageViewController alloc] initWithClear:clear];
        pageViewController.view.clipsToBounds = YES;
        if (clear == YES)
        {
...
//        }
        ...
        PageEntity *startPageEntity = [PageDecoder decode:self.bookEntity.launchPage path:self.rootPath];
        if ([self.currentPageEntity.entityid isEqualToString:startPageEntity.entityid])
        {
            Container *container = (Container *)[self.objects objectAtIndex:i];
        ...
        if (self.isGroupAutoPlay == YES)
        {
            if([self checkContainerInGroups:((Container*)[self.objects objectAtIndex:i]).entity.entityid] == NO)
            {
                [((Container*)[self.objects objectAtIndex:i]) beginView];
            }
        }
        else
        {
...
-(float)startPageTimeWithCom:(Component *)com
{
    ...
    {
        ContainerEntity *entity = com.containerEntity;
...
-(float)animationTimewithContainer:(Container *)con
{
    float aniMaxTime = 0;
...
-(void)setCaseComponent
{
    for (int i = 0 ; i < [self.objects count]; i++)
    {
        Container *caseContainer = (Container*)[self.objects objectAtIndex:i];
        if ([caseContainer.component isKindOfClass:[CaseComponent class]])
        {
            ...
            for (int j = 0; j < containerIdArray.count; j++)
            {
                ...
                {
                    Container *container = (Container*)[self.objects objectAtIndex:c];
...
-(void) playGroup:(NSNumber *) index
{
    ...
    if ([index intValue] < [self.currentPageEntity.groups count])
    {
        ...
        {
            Container* container  = [self getContainerByID:[gp objectAtIndex:i]];
...
//    NSLog(@"pageID = %@\nCount = %d\npageController = %@",self.pageViewController.pageEntity.entityid,self.objects.count,self);
    for (int i = 0 ; i < [self.objects count] ; i++)
    {
        if([((Container*)[self.objects objectAtIndex:i]).entity.entityid isEqualToString:containterid])
        {
...
-(void) loadCurrentEntityWithCache
{
    ...
    {
        [PageDecoder load:self.currentPageEntity path:self.rootPath];
    }
    if (self.currentPageEntity.background != nil)
    {
        if (self.currentPageEntity.state != pageEntityStatePublic) {
            ...
            Container *container = [ContainerDecoder createContainer:self.currentPageEntity.background pageController:self];
...
//        [self.pageViewController.view addSubview:self.cacheImage];
    ...
    else
    {
        ...
        if (self.currentPageEntity.background != nil)
        {
                ...
                ContainerEntity *background = self.pageViewController.pageEntity.background;
                if (background != nil)
                {
...
-(void) resetAnchorPoint:(Container*)container rotation:(float)rotation
{
...
-(void) clean
{
	...
	{
		Container *obj = [self.objects objectAtIndex:m];
		if ([obj isKindOfClass:[Container class]])
		{
            obj.isCleaned = YES;
            [obj.component clean]; 
...
-(void) removeVideo
{
    for (int i = 0; i < [self.objects count]; i++)
    {
        Container *obj = [self.objects objectAtIndex:i];
...
-(void) stopView
{
    for (int i = 0 ; i < [self.objects count]; i++)
	{
		Container *container = [self.objects objectAtIndex:i];
