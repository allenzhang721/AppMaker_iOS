...
#import "HLASIDownloadCache.h"
#import "HLASIHTTPRequest.h"
#import <CommonCrypto/CommonHMAC.h>

static HLASIDownloadCache *sharedCache = nil;

static NSString *sessionCacheFolder = @"SessionStore";
static NSString *permanentCacheFolder = @"PermanentStore";
static NSArray *fileExtensionsToHandleAsHTML = nil;

@interface HLASIDownloadCache ()
+ (NSString *)keyForURL:(NSURL *)url;
- (NSString *)pathToFile:(NSString *)file;
@end

@implementation HLASIDownloadCache

+ (void)initialize
{
	if (self == [HLASIDownloadCache class]) {
...
- (void)updateExpiryForRequest:(HLASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
...
{
  return [HLASIHTTPRequest expiryDateForRequest:request maxAge:maxAge];
}

- (void)storeResponseForRequest:(HLASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	[[self accessLock] lock];
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(HLASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (NSString *)pathToStoreCachedResponseHeadersForRequest:(HLASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (void)removeCachedDataForRequest:(HLASIHTTPRequest *)request
{
	[self removeCachedDataForURL:[request url]];
}

- (BOOL)isCachedDataCurrentForRequest:(HLASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
+ (BOOL)serverAllowsResponseCachingForRequest:(HLASIHTTPRequest *)request
{
...
- (BOOL)canUseCachedDataForRequest:(HLASIHTTPRequest *)request
{
