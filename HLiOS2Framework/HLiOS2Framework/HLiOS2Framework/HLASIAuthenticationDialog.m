...
static HLASIAuthenticationDialog *sharedDialog = nil;
BOOL isDismissingHL = NO;
...
@implementation HLASIAutorotatingViewController
...
@interface HLASIAuthenticationDialog ()
- (void)showTitle;
- (void)show;
...
@implementation HLASIAuthenticationDialog

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [HLASIAuthenticationDialog class]) {
		requestsNeedingAuthentication = [[NSMutableArray array] retain];
	}
}

+ (void)presentAuthenticationDialogForRequest:(HLASIHTTPRequest *)theRequest
{
...
- (UIViewController *)presentingController
{
	if (!presentingController) {
		presentingController = [[HLASIAutorotatingViewController alloc] initWithNibName:nil bundle:nil];
...
- (void)cancelAuthenticationFromDialog:(id)sender
{
	for (HLASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
...
- (NSArray *)requestsRequiringTheseCredentials
{
	...
	for (HLASIHTTPRequest *otherRequest in requestsNeedingAuthentication) {
...
- (void)presentNextDialog
{
	if ([requestsNeedingAuthentication count]) {
		HLASIHTTPRequest *nextRequest = [requestsNeedingAuthentication objectAtIndex:0];
