...
- (void)setMode:(HLMBProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == HLMBProgressHUDModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateProgress {
    [(HLMBRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == HLMBProgressHUDModeDeterminate) {
#if __has_feature(objc_arc)
        self.indicator = [[MoueeMBRoundProgressView alloc] init];
#else
        self.indicator = [[[HLMBRoundProgressView alloc] init] autorelease];
#endif
}
    else if (mode == HLMBProgressHUDModeCustomView && self.customView != nil){
...
+ (HLMBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	HLMBProgressHUD *hud = [[HLMBProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
#if __has_feature(objc_arc)
	return hud;
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[HLMBProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		HLMBProgressHUD *HUD = (HLMBProgressHUD *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"MoueeMBProgressHUDViewIsNillException" 
					format:@"The view used in the MoueeMBProgressHUD initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = HLMBProgressHUDAnimationFade;
        self.mode = HLMBProgressHUDModeIndeterminate;
        self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == HLMBProgressHUDAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == HLMBProgressHUDAnimationZoom) {
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetGrayFillColor(context, 1.0f, 0);
...
@implementation HLMBRoundProgressView
...
- (id)init {
    return [self initWithFrame:CGRectMake(0.0f, 0.0f, 48.0f, 48.0f)];
}
