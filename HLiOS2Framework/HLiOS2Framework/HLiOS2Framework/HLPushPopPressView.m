...
#define kPSFullscreenAnimationBounce 0
#define kPSEmbeddedAnimationBounceMultiplier 0.05f

@interface HLPushPopPressView() {
...
@implementation HLPushPopPressView
...
//        NSLog(@"%f", pinch.scale);
        if (pinch.scale > 1.0f) {
            [self moveToFullscreenAnimated:YES bounces:NO];
        } else {
            [self alignViewAnimated:YES bounces:NO];
        }
    } else {
        [self alignViewAnimated:YES bounces:NO];
    }
...
- (void)pinchPanRotate:(UIGestureRecognizer *)gesture {
    ...
    switch (gesture.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            if ([self.pushPopPressViewDelegate respondsToSelector: @selector(beginPinch)]) {
                [self.pushPopPressViewDelegate beginPinch];
            }
            break;
        }
        ...
        case UIGestureRecognizerStateEnded: {
            anchorPointUpdated = NO;
            [self endedGesture:gesture];
            if ([self.pushPopPressViewDelegate respondsToSelector: @selector(endPinch)]) {
                [self.pushPopPressViewDelegate endPinch];
            }
            break;
        }
    }
...
- (void)moveToFullscreenWindowAnimated:(BOOL)animated {
    ...
    [self moveToFullscreenAnimated:animated bounces:NO];
}

- (void)moveToOriginalFrameAnimated:(BOOL)animated {
    if (self.isFullscreen == NO) return;

    [self moveViewToOriginalPositionAnimated:animated bounces:NO];
}
