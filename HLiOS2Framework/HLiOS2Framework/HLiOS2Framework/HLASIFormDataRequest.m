...
#import "HLASIFormDataRequest.h"


// Private stuff
@interface HLASIFormDataRequest ()
...
@implementation HLASIFormDataRequest
...
- (void)addFile:(NSString *)filePath withFileName:(NSString *)fileName andContentType:(NSString *)contentType forKey:(NSString *)key
{
	...
	if (!fileExists || isDirectory) {
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomainHL code:ASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"No file exists at %@",filePath],NSLocalizedDescriptionKey,nil]]];
	}
	...
	if (!contentType) {
		contentType = [HLASIHTTPRequest mimeTypeForFileAtPath:filePath];
	}
...
{
	HLASIFormDataRequest *newRequest = [super copyWithZone:zone];
