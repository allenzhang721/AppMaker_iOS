...
- (CGFloat)clampedOffset:(CGFloat)offset
{
    ...
    {
        //Changed  liwei 12-5-16
        if (vertical) 
        {
            return fminf(fmaxf(0.0f, offset), numberOfItems * itemWidth-self.bounds.size.height);
        }
        else 
        {
            return fminf(fmaxf(0.0f, offset), numberOfItems * itemWidth-self.bounds.size.width);
        }
    }
}
...
- (void)step
{
    ...
    if (scrolling)
    {
        ...
        CGFloat delta = [self easeInOut:time];
        if (self.uniformScoll) 
        {
            scrollOffset = startOffset + (endOffset - startOffset) * time;
        }
        else 
        {
