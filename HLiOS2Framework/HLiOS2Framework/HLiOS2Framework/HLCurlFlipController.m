...
@interface HLCurlFlipController ()
@end

@implementation HLCurlFlipController
...
- (id)init
{
    ...
    {   
        if (self.curPageController == nil)
        {
            ...
            self.curPageController                    = [[[HLPageController alloc] init] autorelease];
...
- (void)confireGotoPage:(int)index animate:(Boolean)animate{
    ...
    if (!isScrollCurl)
    {
        ...
        HLPageController *curPageController = [[[HLPageController alloc] init] autorelease];
...
-(void) changeToOrientation:(UIInterfaceOrientation)interfaceOrientation
{
    ...
    if(self.curPageController.currentPageEntity.linkPageID != nil)
    {
        ...
        {
            HLPageEntity *entity  = [HLPageDecoder decode:self.curPageController.currentPageEntity.linkPageID path:self.rootPath];
