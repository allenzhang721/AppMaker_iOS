...
@interface HLSliderCoverPageController ()

@end

@implementation HLSliderCoverPageController
...
- (id)init
{
    ...
    if (self) {
        ...
        {
            self.cover1Controller = [[HLPageController alloc] init];
            [self.cover1Controller release];
        }
        if (self.cover2Controller == nil)
        {
            self.cover2Controller = [[HLPageController alloc] init];
            [self.cover2Controller release];
        }
        if (self.cover3Controller == nil)
        {
            self.cover3Controller = [[HLPageController alloc] init];
            [self.cover3Controller release];
        }
        
        pageContainerController = [[HLSliderCoverPageContainersController alloc] init];
    }
    return self;
}
...
{
    self.view = [[[HLClearScrollView alloc] init] autorelease];
    self.view.autoresizingMask = YES;
    self.view.frame = rect;
    clearScrollView = (HLClearScrollView *)self.view;
...
- (void)initEntity:(HLPageEntity *)pageEntity
{
    ...
    HLPageEntity *coverPageEntity = [HLPageDecoder decode:self.currentCoverPageid path:self.currentCoverController.rootPath];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    HLPageController *searchCurrentPage = [self searchCurrentPage];
...
-(void) arrangePage
{
    ...
    HLPageViewController *fp = [self firstPage];
    HLPageViewController *mp = [self middlePage];
    HLPageViewController *lp = [self lastPage];
...
{
    HLPageViewController *coverPageViewCotroller = nil;
    HLPageEntity *coverPageEntity = nil;
    
    if (pageEntity != nil)
    {
        coverPageEntity = [HLPageDecoder decode:pageEntity.beCoveredPageID path:self.currentCoverController.bookEntity.rootPath];
    }
...
{
    float dy   = ((HLClearScrollView *)self.view).contentOffset.y;
...
-(HLPageViewController *) firstPage
{
...
-(HLPageViewController *) middlePage
{
