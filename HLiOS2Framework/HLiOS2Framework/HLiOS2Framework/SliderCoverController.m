...
@implementation SliderCoverController

{
    PageViewController *curpageViewController;
}
...
- (id)init
{
    ...
    if (self) {
        ...
        {
            self.page1Controller = [[SliderCoverPageController alloc] init];
        }
        if (self.page2Controller == nil)
        {
            self.page2Controller = [[SliderCoverPageController alloc] init];
        }
        if (self.page3Controller == nil)
        {
            self.page3Controller = [[SliderCoverPageController alloc] init];
        }
        ...
        containerController = [[SliderCoverContainerController alloc] init];
    }
    return self;
}
...
- (void)setup:(CGRect)rect
{
    ...
    sliderFlipController = (SliderFlipController *)self.flipController;
    
    clearScrollView  =  ((ClearScrollView *)self.viewController.view);
...
- (void)initEntity:(PageEntity *)pageEntity
{
    ...
    PageEntity *coverPageEntity = [PageDecoder decode:self.currentPageid path:self.flipController.bookEntity.rootPath];
...
{
    SliderCoverPageController *coverPageViewCotroller = nil;
    PageEntity *coverPageEntity = nil;
    
    if (pageEntity != nil)
    {
        coverPageEntity = [PageDecoder decode:pageEntity.beCoveredPageID path:self.rootPath];
    }
...
-(SliderCoverPageController *) searchCurrentPage
{
...
-(SliderCoverPageController *) firstPage
{
...
-(SliderCoverPageController *) middlePage
{
    float p1dx = self.page1Controller.view.frame.origin.x;
    float p2dx = self.page2Controller.view.frame.origin.x;
...
-(SliderCoverPageController *) lastPage
{
    float p1dx = self.page1Controller.view.frame.origin.x;
    float p2dx = self.page2Controller.view.frame.origin.x;
